                ...
                <div class="container-fluid">
                    <div class="card shadow mb-4" style="overflow-x: scroll;">
                        <div class="card-header py-3">
                            <h6 class="m-0 font-weight-bold text-primary">List CP Eazy Passport</h6>
                        </div>
                        <div class="card-body">
                            <table class="table table-bordered dataTable">
                                ...
                                <tbody>
                                    <?php 
                                        $no = 1;
                                        $data_eazy = mysqli_query($mysqli, "SELECT * FROM eazy_pas ORDER BY id DESC");
                                        while($eazy = mysqli_fetch_array($data_eazy)){
                                    ?>
                                    <tr>
                                        <td><?php echo $no++; ?></td>
                                        <td><?php echo $eazy['name']; ?></td>
                                        <td><?php echo $eazy['nohp']; ?></td>
                                        <td><?php echo $eazy['statusnya']; ?></td>
                                        <td>
                                            <button class="btn btn-info btn-sm" type="button" data-toggle="modal" data-target="#view_cp<?php echo $eazy['id']; ?>"><i class="fas fa-search fa-sm"></i></button>
                                            <?php
                                                if($eazy['statusnya'] == "Belum Selesai"){
                                            ?>
                                            <a class="btn btn-success btn-sm" href="tuntas_cp.php?id=<?php echo $eazy['id']?>" type="button"><i class="fas fa-check fa-sm"></i></a>
                                            <?php        
                                                }
                                                elseif($eazy['statusnya'] == "Sudah Selesai"){
                                            ?>
                                            <a class="btn btn-warning btn-sm" href="batal_cp.php?id=<?php echo $eazy['id']?>" type="button"><i class="fas fa-times fa-sm"></i></a>
                                            <?php        
                                                }
                                            ?>
                                            <a class="btn btn-danger btn-sm" href="hapus_cp.php?id=<?php echo $eazy['id']?>" type="button"><i class="fas fa-trash fa-sm"></i></a>
                                        </td>
                                    </tr>
                                    <?php
                                        include 'lihat_cp.php';
                                        }
                                    ?>
                                </tbody>    
                            </table>

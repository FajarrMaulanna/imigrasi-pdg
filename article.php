...
<body>
    ...
    <?php
        $id = $_GET['id'];
        $result = mysqli_query($mysqli, "SELECT * FROM post_menu WHERE id = $id");
        while($konten = mysqli_fetch_array($result)){
    ?>
        <div class="container" id="content_vid">
            <h2 class="header_vid"><?php echo $konten['nama_menu'] ?></h2>
            <hr>
            <img id="gambar_post" src="backend/files/gambar_menu/<?php echo $konten['gambarpost'] ?>" alt="">
            <h5 id="submenu"><?php echo $konten['subjudul'] ?></h5>
            <pre id="isipost"><?php echo $konten['isipost'] ?></pre>
        </div>
    <?php
        }
    ?>
